...
public class FacturaPDFView extends AbstractPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		tabla2.addCell(cell);
		tabla2.addCell(mensajes.getMessage("text.cliente.factura.folio").concat(": ").concat(factura.getId().toString()));
		tabla2.addCell(mensajes.getMessage("text.cliente.factura.descripcion").concat(": ").concat(factura.getDescripcion()));
		tabla2.addCell(mensajes.getMessage("text.cliente.factura.fecha").concat(": ").concat(factura.getCreatedAt().toString()));
		...
		cell = new PdfPCell(new Phrase(mensajes.getMessage("text.factura.form.total").concat(": ")));
		cell.setColspan(3);
